...
app.get('/api/company/subscription', (req, res) => {
  try {
    ...
    if (company.isBillingExempt === true || company.tenantType === 'internal') {
      return res.json({
        companyName: company.name, plan: 'internal', status: company.status || 'active',
        billingStatus: 'free', userLimit: 'unlimited', basePrice: 0,
        includedUsers: null, extraUserPrice: 0, activeUsersCount,
        extraUsersCount: 0, monthlyTotal: 0, currency: company.currency || 'EUR',
    ...
    res.json({
      companyName: company.name, plan: company.plan || 'basic',
      status: company.status || 'active', billingStatus: 'active',
      userLimit: null, basePrice, includedUsers, extraUserPrice,
      activeUsersCount, extraUsersCount, monthlyTotal,
      currency: company.currency || 'EUR', note: null
    });
  } catch (e) {
...
// POST /api/platform/companies/:id/admin-user
...
app.use((req, res, next) => {
  if (req.method === 'DELETE') {
    console.log('DEBUG CATCH-ALL: DELETE', req.path, 'user:', req.user?.userId);
  }
  next();
});
...
app.delete('/api/platform/companies/:id', requirePlatformAdmin, (req, res) => {
  console.log('DELETE HANDLER FIRED, id:', req.params.id);
  try {
    const companies = JSON.parse(fs.readFileSync(companiesFile, 'utf8'));
    const idx = companies.findIndex(c => c.id === req.params.id);
    if (idx === -1) return res.status(404).json({ error: 'Įmonė nerasta' });
...
app.get('/api/platform/overview', requirePlatformAdmin, (req, res) => {
  try {
    ...
    res.json({
      ...
      activeCompanies, trialCompanies, suspendedCompanies,
      totalActiveUsers,
      monthlyRecurringRevenue: mrr,
      overduePaymentsCount, upcomingRenewalsCount
    });
  } catch (e) {
...
// PUT /api/platform/payments/:id/mark-paid
