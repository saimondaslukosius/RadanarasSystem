...
// ─────────────────────────────────────────────────────────────────────────────
const STATUS_META = {
  active:    { label: "Aktyvi",      bg: COLOR.greenLight,  color: COLOR.green,  border: COLOR.greenBorder },
  trial:     { label: "Bandomoji",   bg: COLOR.amberLight,  color: COLOR.amber,  border: COLOR.amberBorder },
  suspended: { label: "Sustabdyta",  bg: COLOR.redLight,    color: COLOR.red,    border: COLOR.redBorder   },
  deleted:   { label: "Ištrinta",    bg: "#f1f5f9",         color: "#64748b",    border: "#cbd5e1"         },
  paid:      { label: "Apmokėta",    bg: COLOR.greenLight,  color: COLOR.green,  border: COLOR.greenBorder },
  pending:   { label: "Laukiama",    bg: COLOR.amberLight,  color: COLOR.amber,  border: COLOR.amberBorder },
  overdue:   { label: "Vėluoja",     bg: COLOR.redLight,    color: COLOR.red,    border: COLOR.redBorder   },
  scheduled: { label: "Suplanuota",  bg: COLOR.slateLight,  color: COLOR.slate,  border: COLOR.slateBorder },
};
...
const planLabel = p => ({ basic: "Bazinis", pro: "Pro", enterprise: "Enterprise", free: "Nemokamas", internal: "Vidinis" }[p] || p || "—");
...
const ACTIVITY_META = {
  ...
  company_deleted:          { label: "Ištrinta įmonė",      dot: "#94a3b8", bg: "#f1f5f9", color: "#475569"  },
  company_restored:         { label: "Atkurta įmonė",       dot: "#16a34a", bg: "#f0fdf4", color: "#166534"  },
  company_billing_updated:  { label: "Billing pakeistas",    dot: "#7c3aed", bg: "#f5f3ff", color: "#5b21b6"  },
  user_created:             { label: "Sukurtas vartotojas",  dot: "#3b82f6", bg: "#eff6ff", color: "#1d4ed8"  },
  user_updated:             { label: "Atnaujintas vartotojas",dot: "#94a3b8",bg: "#f8fafc", color: "#475569"  },
  user_disabled:            { label: "Išjungtas vartotojas", dot: "#ef4444", bg: "#fef2f2", color: "#991b1b"  },
  user_password_reset:      { label: "Slaptažodis atstatytas",dot:"#f59e0b", bg: "#fffbeb", color: "#92400e"  },
};
...
// Billing mode badge
function BillingBadge({ company }) {
  const bm = company.billingMode || (company.isBillingExempt
    ? (company.tenantType === "internal" || company.plan === "internal" ? "internal" : "free")
    : (company.status === "trial" ? "trial" : "paid"));
  const map = {
    internal: { label: "VIDINĖ",    bg: "#eff6ff", color: "#1d4ed8", border: "#bfdbfe" },
    free:     { label: "NEMOKAMA", bg: "#f0fdf4", color: "#166534", border: "#bbf7d0" },
    trial:    { label: "BANDOMOJI",bg: "#fffbeb", color: "#854d0e", border: "#fef08a" },
    paid:     { label: "MOKAMA",   bg: "#f0fdf4", color: "#166534", border: "#bbf7d0" },
  };
  const m = map[bm] || map.paid;
  return (
    <span style={{
...
function InternalBadge() { return null; }
...
function CompaniesTab() {
  ...
  // Delete modal state
  const [deleteTarget,  setDeleteTarget]  = useState(null); // company object
  const [deleteConfirm, setDeleteConfirm] = useState("");   // typed name
  const [deleting,      setDeleting]      = useState(false);
  // Edit modal state
  const [formDirty,     setFormDirty]     = useState(false);
  const [toast,         setToast]         = useState(null); // { msg }
  // User management modal state
  const [adminTab,      setAdminTab]      = useState("users");   // "users" | "create"
  const [companyUsers,  setCompanyUsers]  = useState([]);
  const [usersLoading,  setUsersLoading]  = useState(false);
  const [usersLoadErr,  setUsersLoadErr]  = useState(null);       // klaidos žinutė arba null
  const [tmpPwModal,    setTmpPwModal]    = useState(null);       // { password }
  const [userActioning, setUserActioning] = useState(null);       // userId being actioned

  const emptyCompanyForm = {
    name: "", code: "", vat: "", billingEmail: "", country: "", phone: "",
    address: "", notes: "",
    plan: "basic", status: "active",
    billingMode: "paid", tenantType: "customer",
    isBillingExempt: false, unlimitedUsers: false,
    userLimit: null, trialEndsAt: "", billingStatus: "active",
  ...
  const load = (inclDeleted = false) => {
    setLoading(true); setLoadErr("");
    const url = `${API_BASE}/api/platform/companies${inclDeleted ? "?include_deleted=true" : ""}`;
    fetch(url)
      .then(r => r.json())
  ...
  useEffect(() => { load(); }, []);
  // Kai perjungiamas į "deleted" filtrą — reload su include_deleted=true
  const prevStatusFilter = useRef("all");
  useEffect(() => {
    if (statusFilter === prevStatusFilter.current) return;
    prevStatusFilter.current = statusFilter;
    if (statusFilter === "deleted") load(true);
    else load(false);
  }, [statusFilter]);
  ...
  const openNew  = () => {
    ...
    setFormDirty(false); setShowAdminForm(null); setShowForm(true);
  };
  const openEdit = c => {
    setEditCompany(c);
    // Derive billingMode from stored data (backward compat)
    let bm = c.billingMode || "paid";
    if (!c.billingMode) {
      if (c.tenantType === "internal" || (c.isBillingExempt && c.plan === "internal")) bm = "internal";
      else if (c.isBillingExempt && c.status !== "trial") bm = "free";
      else if (c.status === "trial") bm = "trial";
    }
    setCompanyForm({
      name:            c.name           || "",
      code:            c.code           || "",
      ...
      billingMode:     bm,
      tenantType:      c.tenantType     || "customer",
      isBillingExempt: !!c.isBillingExempt,
      unlimitedUsers:  c.userLimit == null && !!c.isBillingExempt,
      userLimit:       c.userLimit      ?? null,
      trialEndsAt:     c.trialEndsAt    || "",
      billingStatus:   c.billingStatus  || "active",
      basePrice:       c.basePrice      ?? 150,
      includedUsers:   c.includedUsers  ?? 1,
      extraUserPrice:  c.extraUserPrice ?? 50,
      currency:        c.currency       || "EUR",
    });
    setFormDirty(false); setShowAdminForm(null); setShowForm(true);
  };
  const cancelForm = () => { setShowForm(false); setEditCompany(null); setFormDirty(false); };
  ...
  const handleSaveCompany = async () => {
    ...
    try {
      ...
      const savedName = companyForm.name;
      const wasEdit   = !!editCompany;
      cancelForm();
      load(statusFilter === "deleted");
      setToast({ msg: wasEdit ? `Išsaugota: ${savedName}` : `Sukurta: ${savedName}` });
      setTimeout(() => setToast(null), 3500);
    } catch { alert("Serverio klaida"); }
    finally { setSaving(false); }
  };

  // ESC — uždaryti company edit modal
  useEffect(() => {
    if (!showForm) return;
    const handler = e => {
      if (e.key !== "Escape") return;
      if (formDirty) {
        if (window.confirm("Yra neišsaugotų pakeitimų. Uždaryti?")) cancelForm();
      } else {
        cancelForm();
      }
    };
  ...
  const loadCompanyUsers = async (companyId) => {
    ...
    try {
      const res  = await fetch(`${API_BASE}/api/platform/companies/${companyId}/users`);
      const data = await res.json();
      if (!res.ok) {
        setUsersLoadErr(data?.error || `Serverio klaida (${res.status})`);
        return;
      }
      if (Array.isArray(data)) setCompanyUsers(data);
      else setUsersLoadErr("Netikėtas serverio atsakymas");
    } catch (e) {
      console.error("loadCompanyUsers error:", e);
      setUsersLoadErr("Nepavyko prisijungti prie serverio");
    } finally { setUsersLoading(false); }
  };

  const openAdminForm = id => {
    setShowAdminForm(id);
  ...
  const cancelAdminForm = () => { setShowAdminForm(null); setTmpPwModal(null); };

  // ESC — uždaryti admin modal
  useEffect(() => {
    if (!showAdminForm) return;
    const handler = e => { if (e.key === "Escape") setShowAdminForm(null); };
    window.addEventListener("keydown", handler);
    return () => window.removeEventListener("keydown", handler);
  }, [showAdminForm]);
  ...
  const handleSaveAdmin = async () => {
    ...
    // User limit check
    const targetCo = companies.find(c => c.id === showAdminForm);
    if (targetCo && !targetCo.unlimitedUsers && targetCo.userLimit != null) {
      if (companyUsers.filter(u => u.status !== "disabled").length >= targetCo.userLimit) {
        alert("Pasiektas vartotojų limitas. Padidinkite limitą arba išjunkite neaktyvius vartotojus.");
        return;
      }
    }
    setSaving(true);
    try {
      ...
      setAdminForm(emptyAdminForm);
      setAdminTab("users");
      await loadCompanyUsers(showAdminForm);
      load();
    } catch { alert("Serverio klaida"); }
    finally { setSaving(false); }
  };

  // ── User actions (role, disable, restore, reset-password) ────────────────────
  const handleUserRoleChange = async (userId, newRole) => {
    setUserActioning(userId);
    try {
      const res  = await fetch(`${API_BASE}/api/platform/users/${userId}`, {
    ...
    finally { setUserActioning(null); }
  };

  const handleUserDisable = async (user) => {
    if (!window.confirm(`Išjungti vartotoją „${user.name}"? Jis nebegalės prisijungti.`)) return;
    setUserActioning(user.id);
    try {
      const res  = await fetch(`${API_BASE}/api/platform/users/${user.id}`, { method: "DELETE" });
      const data = await res.json();
      if (!res.ok) { alert(data.error || "Klaida"); return; }
      setCompanyUsers(prev => prev.map(u => u.id === user.id ? { ...u, status: "disabled", active: false, disabledAt: data.disabledAt } : u));
      load();
    } catch { alert("Serverio klaida"); }
    finally { setUserActioning(null); }
  };
  ...
  const handleUserRestore = async (user) => {
    setUserActioning(user.id);
    try {
      const res  = await fetch(`${API_BASE}/api/platform/users/${user.id}`, {
        method: "PUT", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ status: "active", active: true, disabledAt: null }),
      });
      const data = await res.json();
      if (!res.ok) { alert(data.error || "Klaida"); return; }
      setCompanyUsers(prev => prev.map(u => u.id === user.id ? { ...u, status: "active", active: true, disabledAt: null } : u));
      load();
    } catch { alert("Serverio klaida"); }
    finally { setUserActioning(null); }
  };
  ...
  const handleUserResetPassword = async (user) => {
    if (!window.confirm(`Atstatyti slaptažodį vartotojui „${user.name}"? Bus sugeneruotas laikinas slaptažodis.`)) return;
    setUserActioning(user.id);
    try {
      const res  = await fetch(`${API_BASE}/api/platform/users/${user.id}/reset-password`, { method: "POST" });
      const data = await res.json();
      if (!res.ok) { alert(data.error || "Klaida"); return; }
      setTmpPwModal({ password: data.temporaryPassword, userName: user.name });
    } catch { alert("Serverio klaida"); }
    finally { setUserActioning(null); }
  };
  ...
  const handleRestore = async (company) => {
    if (!window.confirm(`Atkurti įmonę „${company.name}"? Ji taps aktyvi ir jos vartotojai galės prisijungti.`)) return;
    setActioning(company.id);
    try {
      const res  = await fetch(`${API_BASE}/api/platform/companies/${company.id}/restore`, { method: "PUT" });
      const data = await res.json();
      if (!res.ok) { alert(data.error || "Klaida atkuriant"); return; }
      // Po atkūrimo — perjungti į "all" kad matytume atkurtą įmonę aktyvių sąraše
      if (statusFilter === "deleted") {
        setStatusFilter("all"); // useEffect pakraus be include_deleted
      } else {
        load(false);
      }
    } catch { alert("Serverio klaida"); }
  ...
  // ── Delete (soft) ────────────────────────────────────────────────────────────
  const openDeleteModal  = c  => { setDeleteTarget(c); setDeleteConfirm(""); };
  const closeDeleteModal = () => { setDeleteTarget(null); setDeleteConfirm(""); };
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      const res  = await fetch(`${API_BASE}/api/platform/companies/${deleteTarget.id}`, { method: "DELETE" });
      const data = await res.json();
      if (!res.ok) { alert(data.error || "Klaida trinant"); return; }
      closeDeleteModal();
      // Po ištrynimo — automatiškai perjungti į "Ištrintos" filtrą
      if (statusFilter === "deleted") {
        load(true); // jau esame deleted filtre — tiesiog perkrauname
      } else {
        setStatusFilter("deleted"); // perjungiame → useEffect iškvies load(true)
      }
    } catch { alert("Serverio klaida"); }
  ...
  const filtered = companies.filter(c => {
    ...
    const matchStatus = statusFilter === "all"
      ? c.status !== "deleted"          // pagal nutylėjimą slėpti deleted
  ...
  // ── Summary stats (deleted neįskaičiuojamos) ─────────────────────────────────
  const activeOnly   = companies.filter(c => c.status !== "deleted");
  // MRR: tik paid + active + !isBillingExempt įmonės
  const totalMRR     = activeOnly.reduce((s, c) => {
    const isPaid = (c.billingMode || (c.isBillingExempt ? "free" : "paid")) === "paid";
    return s + (isPaid && !c.isBillingExempt && c.status === "active" ? Number(c.monthlyTotal||0) : 0);
  }, 0);
  const countActive  = activeOnly.filter(c => c.status === "active").length;
  const countTrial   = activeOnly.filter(c => c.status === "trial").length;
  const countSusp    = activeOnly.filter(c => c.status === "suspended").length;
  const countDeleted = companies.filter(c => c.status === "deleted").length;
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: "12px", flexWrap: "wrap", marginBottom: "28px" }}>
        <StatChip label="Viso"        value={activeOnly.length} color={COLOR.blue}  bg={COLOR.blueLight}  border={COLOR.blueBorder}  />
        ...
        <StatChip label="Ištrintos"   value={countDeleted}      color="#64748b" bg="#f1f5f9" border="#cbd5e1" />
      ...
      {showForm && (
        ...
        >
          <div style={{
            ...
            boxShadow: "0 24px 80px rgba(0,0,0,0.28)", display: "flex", flexDirection: "column",
            fontFamily: "'Inter','Segoe UI',Arial,sans-serif",
          }}>

            {/* ── Header ──────────────────────────────────────────────────────── */}
            <div style={{
              padding: "20px 28px 16px", borderBottom: `1px solid ${COLOR.slateBorder}`,
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                ...
                <div>
                  <div style={{ fontWeight: 800, fontSize: "16px", color: COLOR.text }}>
                    {editCompany ? "Redaguoti įmonę" : "Nauja įmonės paskyra"}
                  </div>
                  {editCompany && (
                  ...
                  {formDirty && (
                    <div style={{ fontSize: "11px", color: COLOR.amber, fontWeight: 600, marginTop: "1px" }}>
                      Neišsaugotų pakeitimų
                    </div>
                  )}
                </div>
              </div>
              <button
                type="button"
                style={{ ...btnGhost, padding: "5px 11px", fontSize: "16px", lineHeight: 1, flexShrink: 0 }}
                onClick={() => {
                  if (formDirty) {
                    if (window.confirm("Yra neišsaugotų pakeitimų. Uždaryti?")) cancelForm();
                  } else {
            ...
            {/* ── Body ────────────────────────────────────────────────────────── */}
            <div style={{ padding: "24px 28px", display: "flex", flexDirection: "column", gap: "26px" }}>

              {/* A) Pagrindiniai duomenys */}
              <div>
                <div style={{ fontSize: "11px", fontWeight: 700, color: COLOR.muted, textTransform: "uppercase", letterSpacing: "0.7px", marginBottom: "14px", paddingBottom: "8px", borderBottom: `1px solid ${COLOR.slateBorder}` }}>
              ...
              <div>
                ...
                {/* billingMode warning banner */}
                {(companyForm.billingMode === "free" || companyForm.billingMode === "internal") && (
                  <div style={{ background: "#fffbeb", border: "1px solid #fcd34d", borderRadius: "8px", padding: "10px 14px", marginBottom: "14px", fontSize: "13px", color: "#92400e", display: "flex", gap: "8px", alignItems: "flex-start" }}>
                    <span style={{ flexShrink: 0 }}>⚠️</span>
                    <span>
                      {companyForm.billingMode === "internal"
                        ? "Internal paskyra — apmokėjimas neskaičiuojamas, vartotojai neriboti."
                        : "Nemokama paskyra — apmokėjimas neskaičiuojamas."}
                    </span>
                  </div>
                )}
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "14px" }}>
                  ...
                  {/* Billing mode select */}
                  <div style={fGroup}>
                    <label style={lbl}>Billing režimas</label>
                    <select style={inp} value={companyForm.billingMode}
                      onChange={e => {
                        const bm = e.target.value;
                        setFormDirty(true);
                        setCompanyForm(f => {
                          const next = { ...f, billingMode: bm };
                          if (bm === "free") {
                            next.isBillingExempt = true; next.unlimitedUsers = true;
                            next.userLimit = null; next.basePrice = 0;
                            next.extraUserPrice = 0; next.includedUsers = null;
                            next.plan = "free"; next.tenantType = "customer";
                          } else if (bm === "internal") {
                            next.isBillingExempt = true; next.unlimitedUsers = true;
                            next.userLimit = null; next.basePrice = 0;
                            next.extraUserPrice = 0; next.includedUsers = null;
                            next.plan = "internal"; next.tenantType = "internal";
                          } else if (bm === "trial") {
                            next.isBillingExempt = true; next.status = "trial";
                            next.tenantType = "customer"; next.unlimitedUsers = false;
                          } else {
                            // paid
                            next.isBillingExempt = false; next.tenantType = "customer";
                            next.status = f.status === "trial" ? "active" : f.status;
                            if (!next.plan || next.plan === "free" || next.plan === "internal") next.plan = "basic";
                            if (!next.basePrice) next.basePrice = 150;
                            if (!next.includedUsers) next.includedUsers = 1;
                            if (!next.extraUserPrice) next.extraUserPrice = 50;
                          }
                          return next;
                        });
                      }}>
                      <option value="paid">Mokama įmonė</option>
                      <option value="trial">Bandomasis laikotarpis</option>
                      <option value="free">Nemokama / be limitų</option>
                      <option value="internal">Vidinė sistemos paskyra</option>
                    </select>
                  </div>
                  ...
                  <div style={fGroup}>
                    ...
                    <select style={inp} value={companyForm.status}
                      onChange={e => { setCompanyForm(f => ({ ...f, status: e.target.value })); setFormDirty(true); }}>
                  ...
                  <div style={fGroup}>
                    ...
                    <select style={inp} value={companyForm.plan}
                      onChange={e => { setCompanyForm(f => ({ ...f, plan: e.target.value })); setFormDirty(true); }}>
                      <option value="basic">Bazinis</option>
                      <option value="pro">Pro</option>
                      <option value="enterprise">Enterprise</option>
                      <option value="free">Nemokamas</option>
                      <option value="internal">Vidinis</option>
                    </select>
                  </div>

                  {/* Trial ends at — only when trial */}
                  {companyForm.billingMode === "trial" && (
                    <div style={fGroup}>
                      <label style={lbl}>Trial pabaigos data *</label>
                      <input style={inp} type="date" value={companyForm.trialEndsAt}
                        onChange={e => { setCompanyForm(f => ({ ...f, trialEndsAt: e.target.value })); setFormDirty(true); }}
                        onFocus={e => { e.target.style.borderColor = COLOR.blueMid; }}
                        onBlur={e  => { e.target.style.borderColor = COLOR.slateBorder; }}
                      />
                    </div>
                  ...
                  {/* User limit */}
                  <div style={fGroup}>
                    <label style={lbl}>Vartotojų limitas</label>
                    <input style={{ ...inp, opacity: companyForm.unlimitedUsers ? 0.4 : 1 }}
                      type="number" min="1"
                      value={companyForm.unlimitedUsers ? "" : (companyForm.userLimit ?? "")}
                      placeholder={companyForm.unlimitedUsers ? "Neribota" : "pvz. 10"}
                      disabled={companyForm.unlimitedUsers}
                      onChange={e => { setCompanyForm(f => ({ ...f, userLimit: e.target.value ? Number(e.target.value) : null })); setFormDirty(true); }}
                      onFocus={e => { e.target.style.borderColor = COLOR.blueMid; }}
                      onBlur={e  => { e.target.style.borderColor = COLOR.slateBorder; }}
                    />
                  </div>
                ...
                <div style={{ display: "flex", gap: "24px", marginTop: "14px", flexWrap: "wrap" }}>
                  <label style={{ display: "flex", alignItems: "center", gap: "8px", cursor: "pointer", fontSize: "13px", fontWeight: 600, color: COLOR.slate }}>
                    <input type="checkbox" checked={!!companyForm.unlimitedUsers}
                      onChange={e => { setCompanyForm(f => ({ ...f, unlimitedUsers: e.target.checked, userLimit: e.target.checked ? null : f.userLimit })); setFormDirty(true); }}
                    />
                    Neriboti vartotojai
                  </label>
                  <label style={{ display: "flex", alignItems: "center", gap: "8px", cursor: "pointer", fontSize: "13px", fontWeight: 600, color: COLOR.slate }}>
                    <input type="checkbox" checked={!!companyForm.isBillingExempt}
                      onChange={e => { setCompanyForm(f => ({ ...f, isBillingExempt: e.target.checked })); setFormDirty(true); }}
                    />
                    Neskaičiuoti apmokėjimo
                  </label>
                </div>

              </div>

              {/* C) Apmokėjimo detalės — rodoma tik paid/trial */}
              {(companyForm.billingMode === "paid" || companyForm.billingMode === "trial") && (
              <div>
                <div style={{ fontSize: "11px", fontWeight: 700, color: COLOR.muted, textTransform: "uppercase", letterSpacing: "0.7px", marginBottom: "14px", paddingBottom: "8px", borderBottom: `1px solid ${COLOR.slateBorder}` }}>
                  Apmokėjimo detalės
                </div>
                  ...
                  {[
                    ...
                    <div key={k} style={fGroup}>
                      ...
                      <input style={inp} type="number" min="0" value={companyForm[k] ?? ""}
                        onChange={e => { setCompanyForm(f => ({ ...f, [k]: Number(e.target.value) })); setFormDirty(true); }}
                        onFocus={e => { e.target.style.borderColor = COLOR.blueMid; }}
                        onBlur={e  => { e.target.style.borderColor = COLOR.slateBorder; }}
                      />
                    </div>
              ...
              <div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
                  <div style={fGroup}>
                    <label style={lbl}>Adresas</label>
                    <input style={inp} type="text" value={companyForm.address}
                      placeholder="Gatvė, miestas, šalis"
                      onChange={e => { setCompanyForm(f => ({ ...f, address: e.target.value })); setFormDirty(true); }}
                      onFocus={e => { e.target.style.borderColor = COLOR.blueMid; }}
                      onBlur={e  => { e.target.style.borderColor = COLOR.slateBorder; }}
                    />
                  </div>
                  <div style={fGroup}>
                    <label style={lbl}>Pastabos (nematomos klientui)</label>
                    <textarea style={{ ...inp, minHeight: "80px", resize: "vertical", lineHeight: 1.55 }}
                      value={companyForm.notes}
                      placeholder="Vidinė informacija apie šią įmonę…"
                      onChange={e => { setCompanyForm(f => ({ ...f, notes: e.target.value })); setFormDirty(true); }}
                      onFocus={e => { e.target.style.borderColor = COLOR.blueMid; }}
                      onBlur={e  => { e.target.style.borderColor = COLOR.slateBorder; }}
                    />
                  </div>
            ...
            {/* ── Footer ──────────────────────────────────────────────────────── */}
            <div style={{
              padding: "14px 28px 22px", borderTop: `1px solid ${COLOR.slateBorder}`,
              ...
              <button
                ...
                onClick={() => {
                  if (formDirty) {
                    if (window.confirm("Yra neišsaugotų pakeitimų. Atšaukti?")) cancelForm();
                  } else {
                    cancelForm();
                  }
                }}
              ...
              <button
                ...
                style={{
                  ...btnSuccess,
                  opacity: (!companyForm.name.trim() || saving) ? 0.45 : 1,
                  cursor:  (!companyForm.name.trim() || saving) ? "not-allowed" : "pointer",
                }}
      ...
      {showAdminForm && (() => {
        const targetCo   = companies.find(c => c.id === showAdminForm);
        const activeUsers = companyUsers.filter(u => u.status !== "disabled");
        const atLimit     = targetCo && !targetCo.unlimitedUsers && targetCo.userLimit != null
                            && activeUsers.length >= targetCo.userLimit;
        ...
        return (
          <div
            ...
            onClick={e => { if (e.target === e.currentTarget) cancelAdminForm(); }}
          >
            <div style={{
              background: "white", borderRadius: "16px", width: "100%", maxWidth: "720px",
              boxShadow: "0 24px 80px rgba(0,0,0,0.28)", display: "flex", flexDirection: "column",
              fontFamily: "'Inter','Segoe UI',Arial,sans-serif", minHeight: "480px",
            }}>

              {/* Header */}
              <div style={{
                padding: "18px 24px 0", borderBottom: `1px solid ${COLOR.slateBorder}`,
              }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "14px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                    ...
                    <div>
                      <div style={{ fontWeight: 800, fontSize: "16px", color: COLOR.text }}>Vartotojų valdymas</div>
                      {targetCo && <div style={{ fontSize: "12px", color: COLOR.muted }}>{targetCo.name}</div>}
                    </div>
                  </div>
                  <button type="button"
                    style={{ ...btnGhost, padding: "5px 11px", fontSize: "16px", lineHeight: 1 }}
                    onClick={cancelAdminForm}>✕</button>
                </div>
                {/* Tabs */}
                <div style={{ display: "flex", gap: "0" }}>
                  {[
                    { key: "users",  label: `👥 Vartotojai (${companyUsers.length})` },
                    { key: "create", label: "➕ Sukurti vartotoją"                    },
                  ].map(t => (
                    <button key={t.key} type="button"
                      onClick={() => setAdminTab(t.key)}
                      style={{
                        padding: "8px 18px", border: "none", background: "transparent", cursor: "pointer",
                        fontWeight: adminTab === t.key ? 800 : 500, fontSize: "13px",
                        color: adminTab === t.key ? COLOR.blue : COLOR.muted,
                        borderBottom: adminTab === t.key ? `3px solid ${COLOR.blueMid}` : "3px solid transparent",
                        marginBottom: "-1px", transition: "color 0.1s, border-color 0.1s",
                      }}>
                      {t.label}
                    </button>
                  ))}
              ...
              {adminTab === "users" && (
                <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px" }}>
                  {/* User limit warning */}
                  {atLimit && (
                    <div style={{ background: "#fef2f2", border: "1px solid #fecaca", borderRadius: "8px", padding: "10px 14px", marginBottom: "14px", fontSize: "13px", color: "#991b1b", display: "flex", gap: "8px" }}>
                      ⚠️ Pasiektas vartotojų limitas ({targetCo.userLimit}). Išjunkite neaktyvius arba padidinkite limitą.
                    </div>
                  )}
                  {targetCo && !targetCo.unlimitedUsers && targetCo.userLimit != null && (
                    <div style={{ fontSize: "12px", color: COLOR.muted, marginBottom: "12px" }}>
                      Aktyvūs: <strong>{activeUsers.length}</strong> / {targetCo.userLimit}
                    </div>
                  )}
                  ...
                  {usersLoading && (
                    <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                      {[1, 2, 3].map(i => (
                        <div key={i} style={{ height: "52px", background: "#f1f5f9", borderRadius: "8px", animation: "pulse 1.5s infinite" }} />
                      ))}
                    </div>
                  )}

                  {/* Load error */}
                  {!usersLoading && usersLoadErr && (
                    <div style={{ padding: "14px 16px", background: COLOR.redLight, color: COLOR.red,
                      border: `1px solid ${COLOR.redBorder}`, borderRadius: "10px", fontSize: "13px",
                      display: "flex", alignItems: "center", justifyContent: "space-between", gap: "12px", marginBottom: "8px" }}>
                      <span>❌ {usersLoadErr}</span>
                      <button onClick={() => loadCompanyUsers(showAdminForm)}
                        style={{ padding: "4px 12px", background: COLOR.red, color: "#fff", border: "none",
                  ...
                  {/* Users table */}
                  {!usersLoading && !usersLoadErr && companyUsers.length === 0 && (
                    <div style={{ textAlign: "center", padding: "48px 0", color: COLOR.muted }}>
                      <div style={{ fontSize: "32px", marginBottom: "10px", opacity: 0.4 }}>👤</div>
                      <div style={{ fontSize: "14px", fontWeight: 600 }}>Vartotojų dar nėra</div>
                      <div style={{ fontSize: "12px", marginTop: "4px" }}>Sukurkite pirmą vartotoją antrame tabe.</div>
                    </div>
                  )}

                  {!usersLoading && companyUsers.length > 0 && (
                    <div style={{ overflowX: "auto", borderRadius: "10px", border: `1px solid ${COLOR.slateBorder}` }}>
                      <table style={{ width: "100%", borderCollapse: "collapse" }}>
                        <thead>
                          <tr>
                            {["Vardas", "El. paštas", "Rolė", "Statusas", "Paskutinis prisij.", "Sukurtas", "Veiksmai"].map((h, i) => (
                        ...
                        <tbody>
                          {companyUsers.map((u, idx) => {
                            const isLast    = idx === companyUsers.length - 1;
                            const cell      = { ...(isLast ? tCellLast : tCell) };
                            const disabled  = u.status === "disabled";
                            const isBusy    = userActioning === u.id;
                            return (
                              <tr key={u.id} style={{ background: disabled ? "#f8fafc" : "white", opacity: disabled ? 0.75 : 1 }}
                                onMouseEnter={e => { if (!disabled) e.currentTarget.style.background = "#f8fafc"; }}
                                onMouseLeave={e => { e.currentTarget.style.background = disabled ? "#f8fafc" : "white"; }}>

                                <td style={cell}>
                                  <div style={{ fontWeight: 700, fontSize: "13px", color: COLOR.text }}>{u.name}</div>
                                  {u.isPlatformAdmin && <div style={{ fontSize: "10px", color: "#7c3aed", fontWeight: 700 }}>Platform Admin</div>}
                                </td>

                                <td style={{ ...cell, fontSize: "12px", color: COLOR.muted }}>{u.email}</td>

                                {/* Role dropdown */}
                                <td style={cell}>
                                  {u.isPlatformAdmin ? (
                                    <span style={{ fontSize: "12px", fontWeight: 700, color: "#7c3aed" }}>Platform Admin</span>
                                  ) : (
                                    <select
                                      style={{ ...inp, padding: "4px 8px", fontSize: "12px", width: "auto", opacity: isBusy ? 0.5 : 1 }}
                                      value={u.role}
                                ...
                                <td style={cell}>
                                  <span style={{
                                    ...
                                    background: disabled ? "#f1f5f9" : COLOR.greenLight,
                                    color:      disabled ? "#64748b"  : COLOR.green,
                                    border:     `1px solid ${disabled ? "#cbd5e1" : COLOR.greenBorder}`,
                                  }}>
                                    {disabled ? "Išjungtas" : "Aktyvus"}
                                  </span>
                                </td>

                                <td style={{ ...cell, fontSize: "12px", color: COLOR.muted }}>
                                  {u.lastLoginAt
                                    ? new Date(u.lastLoginAt).toLocaleDateString("lt-LT")
                                ...
                                <td style={{ ...cell, textAlign: "right" }}>
                                  {u.isPlatformAdmin ? (
                                    <span style={{ fontSize: "11px", color: COLOR.faint }}>—</span>
                                  ) : (
                                    <div style={{ display: "flex", gap: "5px", justifyContent: "flex-end", flexWrap: "wrap" }}>
                                      {/* Reset password */}
                                      <button type="button"
                                        style={{ ...btnBase, background: "#f59e0b", color: "white", padding: "4px 10px", fontSize: "11px", fontWeight: 700, borderRadius: "6px", opacity: isBusy ? 0.5 : 1 }}
                                        disabled={isBusy}
                                        onClick={() => handleUserResetPassword(u)}>
                                        {isBusy ? "…" : "🔑 Reset"}
                                      </button>
                                      {/* Disable / Restore */}
                                      {disabled ? (
                                        <button type="button"
                                          style={{ ...btnSmSuccess, opacity: isBusy ? 0.5 : 1 }}
                                          disabled={isBusy}
                                          onClick={() => handleUserRestore(u)}>
                                          {isBusy ? "…" : "♻ Atkurti"}
                                        </button>
                                      ) : (
                                        <button type="button"
                                          style={{ ...btnBase, background: "#dc2626", color: "white", padding: "4px 10px", fontSize: "11px", fontWeight: 700, borderRadius: "6px", opacity: isBusy ? 0.5 : 1 }}
                                          disabled={isBusy}
                ...
                <div style={{ padding: "22px 24px", display: "flex", flexDirection: "column", gap: "14px" }}>
                  {atLimit && (
                    <div style={{ background: "#fef2f2", border: "1px solid #fecaca", borderRadius: "8px", padding: "10px 14px", fontSize: "13px", color: "#991b1b" }}>
                      ⚠️ Pasiektas vartotojų limitas ({targetCo?.userLimit}). Negalima sukurti naujų vartotojų.
                    </div>
                  )}
                  {[
                    { k: "name",     l: "Vardas Pavardė *", t: "text",     ac: "name"        },
                    { k: "email",    l: "El. paštas *",     t: "email",    ac: "username"     },
                    { k: "password", l: "Slaptažodis *",    t: "password", ac: "new-password" },
                  ].map(({ k, l, t, ac }) => (
                    <div key={k} style={fGroup}>
                      <label style={lbl}>{l}</label>
                      <input style={inp} type={t} value={adminForm[k]} autoComplete={ac}
                  ...
                  <div style={fGroup}>
                    ...
                    <select style={inp} value={adminForm.role}
                      onChange={e => setAdminForm(f => ({ ...f, role: e.target.value }))}>
                      {ROLE_OPTIONS.map(r => <option key={r.value} value={r.value}>{r.label}</option>)}
                    </select>
                  </div>
                  <div style={{ paddingTop: "8px", borderTop: `1px solid ${COLOR.slateBorder}`, display: "flex", gap: "10px", justifyContent: "flex-end" }}>
                    <button type="button" style={btnGhost} onClick={cancelAdminForm} disabled={saving}>Atšaukti</button>
                    <button type="button"
                      style={{
                        ...btnSuccess,
                        opacity: (!adminForm.name || !adminForm.email || !adminForm.password || saving || atLimit) ? 0.45 : 1,
                        cursor:  (!adminForm.name || !adminForm.email || !adminForm.password || saving || atLimit) ? "not-allowed" : "pointer",
                      }}
                      onClick={handleSaveAdmin}
                      disabled={!adminForm.name || !adminForm.email || !adminForm.password || saving || atLimit}>
      ...
      {tmpPwModal && (
        <div style={{
          ...
          <div style={{
            background: "white", borderRadius: "14px", maxWidth: "420px", width: "100%",
            padding: "28px", boxShadow: "0 20px 60px rgba(0,0,0,0.3)",
          }}>
            <div style={{ fontWeight: 800, fontSize: "16px", color: COLOR.text, marginBottom: "6px" }}>🔑 Laikinas slaptažodis</div>
            <div style={{ fontSize: "13px", color: COLOR.muted, marginBottom: "18px" }}>
              Vartotojas: <strong>{tmpPwModal.userName}</strong>. Perduokite šį slaptažodį saugiai — jis rodomas tik vieną kartą.
            </div>
            <div style={{
              background: "#f8fafc", border: `1.5px solid ${COLOR.slateBorder}`, borderRadius: "8px",
              padding: "14px 16px", fontFamily: "monospace", fontSize: "18px", fontWeight: 800,
              color: COLOR.text, letterSpacing: "2px", textAlign: "center", marginBottom: "16px",
              userSelect: "all",
            }}>
              {tmpPwModal.password}
            </div>
            <div style={{ display: "flex", gap: "10px" }}>
              <button type="button"
                ...
                onClick={() => { navigator.clipboard?.writeText(tmpPwModal.password); }}>
                📋 Kopijuoti
              </button>
              <button type="button"
                style={{ ...btnGhost, flex: 1 }}
                onClick={() => setTmpPwModal(null)}>
                Uždaryti
              </button>
            </div>
          </div>
      ...
      <div style={{ display: "flex", gap: "10px", marginBottom: "16px", flexWrap: "wrap", alignItems: "center" }}>
        ...
        {/* Status filter */}
        <select style={{ ...inp, flex: "0 0 auto", width: "160px" }} value={statusFilter} onChange={e => setStatusFilter(e.target.value)}>
          ...
          <option value="deleted">Ištrintos</option>
        </select>
        ...
        <span style={{ marginLeft: "auto", fontSize: "13px", color: COLOR.muted, whiteSpace: "nowrap" }}>
          {statusFilter === "deleted"
            ? `${filtered.length} ištrintų įmonių`
            : filtered.length === activeOnly.length
              ? `${activeOnly.length} įmonių`
              : `Rodoma ${filtered.length} iš ${activeOnly.length}`}
        </span>
      </div>
        ...
        <table style={tbl}>
          ...
          <tbody>
            ...
            {filtered.map((c, idx) => {
              ...
              const rowBg      = isDeleted ? "#f8fafc" : "white";
              const cellStyle  = { ...cell, opacity: isDeleted ? 0.6 : 1 };

              return (
                <tr key={c.id} style={{ background: rowBg }}
                  onMouseEnter={e => { if (!isDeleted) e.currentTarget.style.background = "#f8fafc"; }}
                  ...
                  {/* Status badge */}
                  <td style={cellStyle}><Badge status={c.status} /></td>

                  {/* Plan */}
                  <td style={cellStyle}>
                    {isDeleted
                      ? <span style={{ fontSize: "12px", color: COLOR.faint }}>—</span>
                      : <span style={{
                  ...
                  <td style={{ ...cellStyle, textAlign: "right" }}>
                    ...
                    <div style={{ fontSize: "11px", color: COLOR.muted }}>
                      {c.unlimitedUsers || c.userLimit == null
                        ? <span title="Neribota">∞</span>
                        : `${c.totalUsersCount ?? 0} / ${c.userLimit}`}
                    </div>
                  </td>
                    ...
                    {isDeleted
                      ...
                      : isInternal
                        ...
                        : <><span style={{ fontWeight: 900, fontSize: "15px", color: COLOR.text }}>{fmtEur(c.monthlyTotal)}</span>
                           <div style={{ fontSize: "11px", color: COLOR.muted }}>/ mėn.</div></>}
                  </td>
                  ...
                  <td style={cellStyle}>
                    {c.billingEmail
                      ...
                      : <span style={{ color: COLOR.faint }}>—</span>}
                    {isDeleted && (c.deletedAt || c.deletedBy) && (
                      <div style={{ fontSize: "11px", color: COLOR.faint, marginTop: "4px", lineHeight: 1.5 }}>
                        {c.deletedAt && (
                          <div>🗑 {new Date(c.deletedAt).toLocaleDateString("lt-LT")} {new Date(c.deletedAt).toLocaleTimeString("lt-LT", { hour: "2-digit", minute: "2-digit" })}</div>
                        )}
                        {c.deletedBy && (
                          <div style={{ color: "#94a3b8" }}>Trynė: {c.deletedBy}</div>
                        )}
                      </div>
                    )}
                  </td>
                  ...
                  <td style={{ ...cellStyle, textAlign: "right" }}>
                    {isDeleted ? (
                      ...
                      <div style={{ display: "flex", gap: "6px", justifyContent: "flex-end", flexWrap: "wrap" }}>
                        ...
                        {/* Ištrinti (soft-delete, ne internal) */}
                        {!isInternal && (
                          <button
                            type="button"
                            style={{ ...btnSmGhost, color: "#6b7280", borderColor: "#d1d5db" }}
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "20px" }}>
              <div style={{ width: "42px", height: "42px", borderRadius: "10px", background: "#fef2f2", border: "1px solid #fecaca", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "20px", flexShrink: 0 }}>
                🗑
              </div>
              <div>
                <div style={{ fontWeight: 800, fontSize: "17px", color: COLOR.text }}>Įmonės ištrynimas</div>
                <div style={{ fontSize: "12px", color: COLOR.muted, marginTop: "2px" }}>Soft-delete — duomenys nebus fiziškai ištrinti</div>
              </div>
            </div>

            {/* Warning box */}
            <div style={{ background: "#fef2f2", border: "1px solid #fecaca", borderRadius: "10px", padding: "14px 16px", marginBottom: "20px" }}>
              <div style={{ fontWeight: 700, fontSize: "13px", color: "#991b1b", marginBottom: "4px" }}>⚠️ Šis veiksmas paslėps įmonę iš aktyvių sąrašų</div>
              <div style={{ fontSize: "13px", color: "#b91c1c", lineHeight: 1.55 }}>
                Įmonės vartotojai negalės prisijungti. Duomenys (užsakymai, dokumentai, vartotojai) nebus fiziškai ištrinti ir gali būti atkurti.
              </div>
            </div>

            {/* Company name */}
            <div style={{ background: COLOR.slateLight, border: `1px solid ${COLOR.slateBorder}`, borderRadius: "8px", padding: "12px 16px", marginBottom: "20px" }}>
              <div style={{ fontSize: "11px", fontWeight: 700, color: COLOR.muted, textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: "4px" }}>Trinama įmonė</div>
              <div style={{ fontWeight: 800, fontSize: "15px", color: COLOR.text }}>{deleteTarget.name}</div>
            </div>

            {/* Confirmation input */}
            <div style={{ marginBottom: "24px" }}>
              <label style={{ ...lbl, marginBottom: "8px" }}>
                Patvirtinimui įrašykite tikslų įmonės pavadinimą:
              </label>
              <input
                style={{
                  ...inp,
                  borderColor: deleteConfirm && deleteConfirm.trim() !== deleteTarget.name.trim() ? COLOR.red : COLOR.slateBorder,
                }}
                autoFocus
                placeholder={deleteTarget.name}
                value={deleteConfirm}
                onChange={e => setDeleteConfirm(e.target.value)}
                onKeyDown={e => e.key === "Enter" && deleteConfirm.trim() === deleteTarget.name.trim() && handleDelete()}
              />
              {deleteConfirm && deleteConfirm.trim() !== deleteTarget.name.trim() && (
                <div style={{ fontSize: "12px", color: COLOR.red, marginTop: "6px" }}>
                  Pavadinimas nesutampa
                </div>
              )}
            </div>
            ...
            <div style={{ display: "flex", gap: "10px" }}>
              <button
                ...
                style={{
                  ...btnDanger,
                  flex: 1,
                  opacity: (deleteConfirm.trim() === deleteTarget.name.trim() && !deleting) ? 1 : 0.4,
                  cursor: (deleteConfirm.trim() === deleteTarget.name.trim() && !deleting) ? "pointer" : "not-allowed",
                }}
                disabled={deleteConfirm.trim() !== deleteTarget.name.trim() || deleting}
                onClick={handleDelete}
              >
                {deleting ? "Trinama…" : "🗑 Ištrinti įmonę"}
              </button>
              <button type="button" style={{ ...btnGhost, flex: 1 }} onClick={closeDeleteModal} disabled={deleting}>
                Atšaukti
              </button>
            </div>
          </div>
      ...
      {toast && (
        <div style={{
          ...
          zIndex: 2000, background: "#166534", color: "white",
              ...
              return (
                <tr key={c.id} style={{ background: "white" }}
                  ...
                  <td style={cell}>
                    ...
                    {c.isBillingExempt && (
                      <span style={{ display: "inline-block", marginTop: "3px", padding: "1px 7px", borderRadius: "999px", background: COLOR.greenLight, color: COLOR.green, border: `1px solid ${COLOR.greenBorder}`, fontSize: "10px", fontWeight: 800 }}>VIDINĖ · NEMOKAMA</span>
                    )}
                  </td>
