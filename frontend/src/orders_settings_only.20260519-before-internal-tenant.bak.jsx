...
function SubscriptionTab() {
  ...
  const rows = [
    ...
    { label: "Papildomi vartotojai",     value: data.extraUsersCount,
      highlight: data.extraUsersCount > 0 },
      ...
      <div style={{
        ...
        <div>
          ...
          <div style={{ fontSize: "32px", fontWeight: 800 }}>
            {fmt(data.monthlyTotal)} {data.currency}
          </div>
...
function UsersManager({ authUser }) {
  ...
  const loadUsers = async () => {
    ...
    try {
      ...
      setLoadErr("Nepavyko įkelti vartotojų sąrašo.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const openEdit = (u) => {
    setEditId(u.id);
    setForm({ name: u.name, email: u.email, password: "", role: u.role, status: u.status || "active" });
    setShowForm(true);
  };
  ...
  const handleSave = async () => {
    if (!form.name.trim() || !form.email.trim()) {
      window.alert("Prašome užpildyti: Vardas ir El. paštas");
      return;
    }
    if (!editId && !form.password.trim()) {
      window.alert("Naujo vartotojo slaptažodis yra privalomas");
      return;
    }
    setSaving(true);
    try {
      ...
      const body   = { ...form };
      if (editId && !body.password.trim()) delete body.password;
      const res  = await fetch(url, { method, headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
      const data = await res.json();
      if (!res.ok) { window.alert(data.error || "Klaida išsaugant"); return; }
      await loadUsers();
      cancelForm();
    } catch {
      window.alert("Serverio klaida");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (user) => {
    if (!window.confirm(`Ar tikrai ištrinti vartotoją "${user.name}"?`)) return;
    try {
      const res  = await fetch(`${API_BASE}/api/users/${user.id}`, { method: "DELETE" });
      const data = await res.json();
      if (!res.ok) { window.alert(data.error || "Klaida trinant"); return; }
      await loadUsers();
    } catch {
      window.alert("Serverio klaida");
    }
  ...
  if (loading) return <div style={{ padding: "20px", color: "#64748b" }}>⏳ Kraunama...</div>;
  if (loadErr) return <div style={{ padding: "20px", color: "#dc2626" }}>❌ {loadErr}</div>;

  return (
    <div>
      {/* Perspėjimas apie papildomų vartotojų kainą */}
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
        ...
        {isAdmin && (
          <button type="button" style={btn} onClick={openNew}>+ Naujas vartotojas</button>
        )}
      </div>

      {showForm && (
        <div style={{ background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: "10px", padding: "22px", marginBottom: "24px" }}>
          <h4 style={{ margin: "0 0 16px", color: "#1e3a8a" }}>
            {editId ? "✏️ Redaguoti vartotoją" : "➕ Naujas vartotojas"}
          </h4>
          <div style={formGrid}>
            <div style={formGroup}>
              <label style={label}>Vardas, pavardė *</label>
              <input
                style={inputBase} value={form.name} placeholder="Vardas Pavardė"
            ...
            <div style={formGroup}>
              ...
              <input
                style={inputBase} type="email" value={form.email} placeholder="email@example.com"
            ...
            <div style={formGroup}>
              ...
              <select style={inputBase} value={form.role} onChange={e => setForm(f => ({ ...f, role: e.target.value }))}>
                {ROLE_OPTIONS.map(r => <option key={r.value} value={r.value}>{r.label}</option>)}
              </select>
            </div>
            <div style={formGroup}>
              <label style={label}>Statusas</label>
              <select style={inputBase} value={form.status} onChange={e => setForm(f => ({ ...f, status: e.target.value }))}>
          ...
          <div style={{ display: "flex", gap: "10px", marginTop: "6px" }}>
            <button type="button" style={btnSuccess} onClick={handleSave} disabled={saving}>
        ...
        <table style={table}>
          <thead>
            <tr>
              ...
              <th style={th}>El. paštas</th>
              <th style={th}>Rolė</th>
              <th style={th}>Statusas</th>
              {isAdmin && <th style={th}>Veiksmai</th>}
            </tr>
          </thead>
          <tbody>
            {users.map(user => (
              <tr key={user.id}>
                <td style={td}>
                  ...
                  {user.id === authUser?.userId && (
                    <div style={{ fontSize: "11px", color: "#3b82f6", marginTop: "2px" }}>← jūs</div>
                  )}
                </td>
                <td style={td}>{user.email}</td>
                <td style={td}>{ROLE_LABELS[user.role] || user.role}</td>
                <td style={td}>
                  <span style={{
                    display: "inline-block",
                    padding: "3px 10px", borderRadius: "999px", fontSize: "12px", fontWeight: 700,
                    background: user.status === "inactive" ? "#fee2e2" : "#dcfce7",
                    color:      user.status === "inactive" ? "#991b1b" : "#166534",
                    border:     `1px solid ${user.status === "inactive" ? "#fecaca" : "#bbf7d0"}`
                  }}>
                    {user.status === "inactive" ? "⛔ Neaktyvus" : "✅ Aktyvus"}
                  </span>
                </td>
                {isAdmin && (
                  <td style={td}>
                    <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                      <button
                        type="button"
                        style={{ ...btn, fontSize: "12px", padding: "5px 12px" }}
                      ...
                      <button
                        type="button"
                        style={{ ...btn, background: "#dc2626", boxShadow: "none", fontSize: "12px", padding: "5px 12px" }}
                        onClick={() => handleDelete(user)}
                        disabled={user.id === authUser?.userId}
                        title={user.id === authUser?.userId ? "Negalima ištrinti savo paskyros" : "Ištrinti vartotoją"}
                      >
                        🗑 Ištrinti
                      </button>
                    </div>
            ...
            {users.length === 0 && (
              <tr>
                <td colSpan={isAdmin ? 5 : 4} style={{ ...td, textAlign: "center", color: "#64748b", padding: "32px" }}>
                  Vartotojų nėra
                </td>
              </tr>
            )}
...
export function Modal({ type, initialData, onClose, clients, carriers, saveOrders, saveCarriers, saveClients, orders, settings, persistFutureDomain, persistTarget, workflowMode = "default", prefill = null }) {
  ...
  */
  ...
  // Prefill effect runs AFTER initialization so it overlays on the fresh form structure
  useEffect(() => {
    if (!prefill || initialData) return;
    console.log("PREFILL IN MODAL:", prefill);

    // Resolve client from prefill.clientId or by matching clientName in DB
    const matchedClient = prefill.clientId
      ? clients.find((c) => String(c.id) === String(prefill.clientId))
      : clients.find((c) => (c.name || "").toLowerCase() === (prefill.clientName || "").toLowerCase());

    // VIN notes block to append to cargoNotes
    const vinBlock = prefill.vinNumbers ? "VINs:\n" + prefill.vinNumbers : "";

    setProjectDraft((prev) => {
      const next = {
        ...prev,
        // ── Client ────────────────────────────────────────────────────────
        clientId:           matchedClient ? String(matchedClient.id)           : (prefill.clientId      || prev.clientId),
        clientName:         matchedClient ? (matchedClient.name        || "")  : (prefill.clientName    || prev.clientName),
        clientCompanyCode:  matchedClient ? (matchedClient.companyCode || prev.clientCompanyCode) : prev.clientCompanyCode,
        clientVatCode:      matchedClient ? (matchedClient.vatCode     || prev.clientVatCode)     : prev.clientVatCode,
        clientPhone:        matchedClient ? (matchedClient.phone       || prev.clientPhone)       : (prefill.clientPhone || prev.clientPhone),
        clientEmail:        matchedClient ? (matchedClient.email       || prev.clientEmail)       : (prefill.clientEmail || prev.clientEmail),
        clientAddress:      matchedClient ? (matchedClient.address     || prev.clientAddress)     : prev.clientAddress,
        clientContactName:  prefill.clientContactName || prev.clientContactName,
        clientPrice:        prefill.clientPrice       || prev.clientPrice,
        clientOrderNumber:  prefill.clientOrderNumber || prev.clientOrderNumber,
        // ── Cargo ─────────────────────────────────────────────────────────
        cargoType:   prefill.cargoType  || prev.cargoType,
        quantity:    prefill.quantity   || prev.quantity,
        weight:      prefill.weight     || prev.weight,
        ldm:         prefill.ldm        || prev.ldm,
        vinNumbers:  prefill.vinNumbers || prev.vinNumbers,
        cargoNotes:  [prev.cargoNotes, vinBlock].filter(Boolean).join("\n\n"),
        // ── Loading ───────────────────────────────────────────────────────
        loadingDate:        prefill.loadingDate        || prev.loadingDate,
        loadingTime:        prefill.loadingTime        || prev.loadingTime,
        loadingCompanyName: prefill.loadingCompany     || prev.loadingCompanyName,
        loadingStreet:      prefill.loadingAddress     || prev.loadingStreet,
        loadingCity:        prefill.loadingCity        || prev.loadingCity,
        loadingPostalCode:  prefill.loadingPostalCode  || prev.loadingPostalCode,
        loadingCountry:     prefill.loadingCountry     || prev.loadingCountry,
        loadingContact:     prefill.loadingContact     || prev.loadingContact,
        loadRefLoading:     prefill.loadingRef         || prev.loadRefLoading,
        // ── Unloading ─────────────────────────────────────────────────────
        unloadingDate:        prefill.unloadingDate        || prev.unloadingDate,
        unloadingTime:        prefill.unloadingTime        || prev.unloadingTime,
        unloadingCompanyName: prefill.unloadingCompany     || prev.unloadingCompanyName,
        unloadingStreet:      prefill.unloadingAddress     || prev.unloadingStreet,
        unloadingCity:        prefill.unloadingCity        || prev.unloadingCity,
        unloadingPostalCode:  prefill.unloadingPostalCode  || prev.unloadingPostalCode,
        unloadingCountry:     prefill.unloadingCountry     || prev.unloadingCountry,
        unloadingContact:     prefill.unloadingContact     || prev.unloadingContact,
        loadRefUnloading:     prefill.unloadingRef         || prev.loadRefUnloading,
        // ── Route / notes ─────────────────────────────────────────────────
        route: prefill.route || prev.route,
        notes: [prev.notes, prefill.notes, prefill.rawText ? "Import raw text:\n" + prefill.rawText : ""].filter(Boolean).join("\n\n"),
      };
      console.log("DRAFT AFTER PREFILL:", next);
      return next;
    });
    ...
    setExecutionDraft((prev) => {
      const next = {
        ...prev,
        orderType:    workflowMode === "expedition" ? "resale_to_carrier" : prev.orderType,
        driverName:   prefill.driverName   || prev.driverName,
        truckPlate:   prefill.truckPlate   || prev.truckPlate,
        trailerPlate: prefill.trailerPlate || prev.trailerPlate,
      };
      console.log("EXECUTION AFTER PREFILL:", next);
      return next;
    });

    // Update the client picker search input so the client visually appears selected
    if (matchedClient) {
      const clientLabel = [matchedClient.name, matchedClient.companyCode, matchedClient.vatCode, matchedClient.email].filter(Boolean).join(" | ");
      setClientPickerSearch(clientLabel);
    } else if (prefill.clientName) {
      setClientPickerSearch(prefill.clientName);
    }
  }, [prefill, initialData, workflowMode, clients]);
