...
function OverviewTab() {
  ...
  useEffect(() => {
    fetch(`${API_BASE}/api/platform/overview`)
      .then(r => r.json())
      .then(d => d.totalCompanies !== undefined ? setKpi(d) : setErr(d.error || "Klaida"))
  ...
  const healthItems = [
    { label: "Prisijungimo sistema",  status: "ok",            note: "JWT / bcrypt — veikia"               },
    { label: "Apmokėjimo režimas",    status: "manual",        note: "Rankinis sekimas — be integracijos"  },
    { label: "El. pašto priminimai",  status: "not_connected", note: "El. pašto paslauga neprijungta"      },
    { label: "Mokėjimų sistema",      status: "not_connected", note: "Integracija nesukonfigūruota"        },
  ];
  const healthMeta = {
  ...
  return (
    <div>
      <SectionHead
        title="Platformos apžvalga"
        sub="Realaus laiko rodikliai iš companies.json ir users.json"
      />
      ...
      <div style={{ background: "white", border: `1px solid ${COLOR.slateBorder}`, borderRadius: "14px", overflow: "hidden" }}>
        <div style={{ padding: "16px 24px", borderBottom: `1px solid ${COLOR.slateBorder}`, display: "flex", alignItems: "center", gap: "8px" }}>
          ...
          <span style={{ fontWeight: 800, fontSize: "15px", color: COLOR.text }}>Platformos būsena</span>
        </div>
