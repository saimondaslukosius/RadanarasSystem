...
<div style={formGroup}>
  ...
  <div
    ...
    onDrop={async (e) => {
      ...
      const uploadData = await handleFileUpload(file, "carrier", editingCarrierId || "TEST123");`r`n      const fileUrl = uploadData.fileUrl;
      handleCarrierDocumentChange(doc.id, "link", fileUrl);
      handleCarrierDocumentChange(doc.id, "fileName", file.name);

      handleCarrierDocumentChange(doc.id, "number", uploadData.extracted?.number || "");`r`n      handleCarrierDocumentChange(doc.id, "validUntil", uploadData.extracted?.validUntil || "");
    }}
  >
