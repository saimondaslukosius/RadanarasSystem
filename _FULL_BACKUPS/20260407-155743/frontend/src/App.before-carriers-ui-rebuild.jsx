...
function App() {
  ...
  const handleSaveCarrier = () => {
    ...
    const cleanedDocuments = carrierForm.documents.filter(doc =>
      doc.title.trim() || doc.number.trim() || doc.validUntil.trim() || doc.link.trim()
    );
  ...
  const renderCarriers = () => {
    return (
      <div>
        <div style={pageHeaderRow}>
          <h2 style={{ margin: 0, color: "#1e3a8a" }}>Vežėjai</h2>
          <button style={primaryButton} onClick={openNewCarrierModal}>+ Naujas vežėjas</button>
        </div>
          ...
          <table style={tableStyle}>
            <thead>
              <tr style={{ background: "#f8fafc" }}>
                ...
                <th style={thStyle}>Įmonės kodas</th>
                <th style={thStyle}>PVM kodas</th>
                <th style={thStyle}>El. paštas</th>                <th style={thStyle}>CMR</th>                <th style={thStyle}>Licencija</th>                <th style={thStyle}>Veiksmai</th>
              </tr>
            </thead>
            <tbody>
              {carriers.map((item) => (
                <tr key={item.id}>
                  <td style={tdStyle}>
                    <button onClick={() => openCarrierProfile(item.id)} style={linkButtonStyle}>
                      {item.name}
                    </button>
                  </td>
                  <td style={tdStyle}>{item.carrierType || "-"}</td>
                  <td style={tdStyle}>{item.companyCode || "-"}</td>
                  <td style={tdStyle}>{item.vatCode || "-"}</td>
                  <td style={tdStyle}>{item.email || "-"}</td>                  <td style={tdStyle}>{getDaysLeft(item.documents?.find(doc => doc.title === "CMR draudimas")?.validUntil)}</td>                  <td style={tdStyle}>{getDaysLeft(item.documents?.find(doc => doc.title === "Transporto licencija")?.validUntil)}</td>
                  <td style={tdStyle}>
                    ...
                    <button style={dangerActionButton} onClick={() => handleDeleteCarrier(item.id)}>Ištrinti</button>
                  </td>
                </tr>
              ))}
            </tbody>
              ...
              <div key={doc.id} style={contactCard}>
                ...
                <div style={twoCol}>
                  ...
                  <div style={formGroup}>
                    ...
                    <input style={inputStyle} value={doc.link} onChange={(e) => handleCarrierDocumentChange(doc.id, "link", e.target.value)} />
                  </div>
                </div>
              </div>
            ))}
